        ...
        for filename in files:
            if re.match('.*\d+_\d+.txt', filename):
                ...
                with open(filepath, 'r') as f:
                    for line in f:
                        if six.PY2:
                            tokenize_words = tokenizer.tokenize(line.decode('utf-8').strip())
                        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-trd', '--train_dir', type = str, nargs = '?', \
            default = './aclImdb/train/', help = 'train data directory')
    parser.add_argument('-ted', '--test_dir', type = str, nargs = '?', \
            default = './aclImdb/test/', help = 'test data directory')
    parser.add_argument('-wip', '--word_index_path', type = str, nargs = '?', \
            default = './aclImdb_word_index.json', help = 'word index file path')
    parser.add_argument('-onz', '--output_npz', type = str, nargs = '?', \
            default = './aclImdb.npz', help = 'output npz')
    ...
    np.savez(output_npz, x_train = np.asarray(train_x),
