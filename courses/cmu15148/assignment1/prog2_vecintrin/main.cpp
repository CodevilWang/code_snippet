...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  __cmu418_vec_float all_one = _cmu418_vset_float(1.0);
  __cmu418_vec_float all_max_value = _cmu418_vset_float(9.999999f);
  __cmu418_mask maskZero, maskNoneZero, maskOne;
  ...
  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    ...
    maskZero = _cmu418_init_ones(0);
    _cmu418_veq_int(maskZero, y, zero, maskAll);
    maskNoneZero = _cmu418_mask_not(maskZero);

    while (_cmu418_cntbits(maskNoneZero)) {
        _cmu418_vmult_float(result, result, x, maskNoneZero);
        _cmu418_vsub_int(y, y, one, maskNoneZero);
        maskZero = _cmu418_init_ones(0);
        _cmu418_veq_int(maskZero, y, zero, maskAll);
        maskNoneZero = _cmu418_mask_not(maskZero);
    }
    __cmu418_mask maskMax = _cmu418_init_ones(0);
    _cmu418_vgt_float(maskMax, result, all_max_value, maskAll);
    _cmu418_vset_float(result, 9.999999f, maskMax);
    _cmu418_vstore_float(output+i, result, maskAll);
  }

  int remainer = N % VECTOR_WIDTH;
  if (remainer > 0) {
    clampedExpSerial(values + (N - remainer), exponents + (N - remainer), output + (N - remainer), remainer);
  }
}
